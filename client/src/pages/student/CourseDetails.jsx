...
const CourseDetails = () => {
  ...
  return courseData ? (
    ...
    justify-between md:px-36 px-8 md:pt-30 pt-20 text-left'>
        <div className='absolute top-0 left-0 w-full h-section-height bg-gradient-to-b from-orange-200/70'></div>
        {/* //left coloumn */}
        <div className='max-w-xl z-10 text-gray-800'>
          ...
          <div className='flex items-center space-x-2 pt-3 pb-1'>
            ...
            <p className='text-orange-500'>({courseData.courseRatings.length}{courseData.courseRatings.length > 1 ? '-ratings' : '-ratings'})</p>
            <p>{courseData.enrolledStudents.length}{courseData.enrolledStudents.length > 1 ? '-students' : '-students'}</p>
          </div>
          <p className='text-sm'>Course by <span className='text-blue-600 underline'>Sugoi Learn</span></p>
          <div className='pt-8 text-gray-800'>
            <h2 className='text-xl font-semibold'>Course Structure</h2>
            <div className='pt-5'>
              {courseData.courseContent.map((chapter, index) => (
                <div key={index} className='border border-gray-600 bg-orange-200/70 mb-2 rounded'>
                  ...
                  <div className={`overflow-hidden transition-all duration-300 ${openSection[index] ? 'max-h-96' : 'max-h-0'}`}>
                    <ul className='list-disc md:pl-10 pl-4 pr-4 py-2 texr-gray-600 border-t border-gray-600'>
        ...
        {/* //Right coloumn */}
        <div className='max-w-course-card z-10 shadow-custom-card rounded-t md:rounded-none overflow-hidden bg-orange min-w-[300px] sm:min-w-[420px]'>
          {
            playerData ?
          ...
          <div className='p-5'>
            ...
            <div className='flex gap-3 items-center pt-2'>
              ...
              <p className='md:text-lg text-gray-600 line-through'>{currency}{courseData.coursePrice}</p>
              <p className='md:text-lg text-gray-600'>{courseData.discount}% off</p>
            </div>
            ...
            <button className='md:mt-6 mt-4 w-full py-3 roundedd bg-blue-600 text-white font-medium'
